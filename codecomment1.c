// MPU6050 Accelerometer and Gyroscope Sensor configration

#include <math.h>      //upload different mathmatical functions
#include "mpu6050.h"   //upload sensor defined file
...
const uint16_t I2C_TIMEOUT = 100;       //bus timeout value
const double Acc_Z_corrector = 14418.0; //z axis corrector value
uint32_t timer;                         //initialize 32 bit timer

//define tuning value for x axis
Filter_t FilterX = {
    .Q_ANGLE = 0.001f,
    .Q_BIAS = 0.003f,
    .R_MEASURE = 0.03f};

//define tuning value for y axis
Filter_t FilterY = {
    .Q_ANGLE = 0.001f,
    .Q_BIAS = 0.003f,
    .R_MEASURE = 0.03f,
...
uint8_t MPU_Init(I2C_HandleTypeDef *I2Cx)
{
    ...
    // TODO: If check = 104 fails return 1 else confirm device wirh given data
    if (check == 104) 
    {
        Data = 0;
...
/*initialize slave address for accelator readings
Set the accelerometer full scale range 
I2Cx Pointer to I2C structure config */
...
/*initialize slave address for gyroscope readings 
...
// initialize slave address for tempreature readings
